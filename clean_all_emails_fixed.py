#!/usr/bin/env python3
"""
Fixed Batch Email Cleaner for NLP Training

This script cleans all email CSV files in the current directory using the fixed cleaner.
...
Usage:
python clean_all_emails_fixed.py
"""

import os
import glob
from email_cleaner_fixed import process_csv_file
...
def clean_all_email_files():
    ...
    print("🧹 Fixed Email Cleaning Process Started")
    print("=" * 50)
    ...
    for input_file in files_to_clean:
        if os.path.exists(input_file):
            ...
            output_file = f"{name}_cleaned_fixed{ext}"
    ...
    cleaned_files = glob.glob("*_cleaned_fixed.csv")
    ...
    print("5. All emails should now be processed correctly")
